...
**The dependency-manifest probe.** The cheapest reliable test for the bar is the project's own manifest: a thing that calls a model has a model client. Open `package.json` / `Cargo.toml` / `go.mod` and look for an Anthropic, OpenAI, Vercel AI SDK, or equivalent HTTP-to-a-model dependency. If none is there, the project is not making model calls — it is wrapping something that does, and the loop worth visualizing belongs to that something. The probe is fast, and it decided three of the five exclusions below.

**It is a strong prior, not a verdict** — it detects a *dependency*, not a *capability*. A project can hand-roll an HTTP client against a model API and leave its manifest clean. `superplanehq/superplane` was evaluated for the group below and does exactly that: no LLM SDK anywhere in `go.mod`, but a first-party Anthropic client at `pkg/agents/anthropic/client.go` and a real tool-dispatch loop in `pkg/workers/agent_stream_worker.go` (`streamProviderTurn`). A clean manifest earns a grep for the vendor's API hostname and for SSE/streaming plumbing before it earns a conclusion.
