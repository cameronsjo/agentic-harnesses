...
- **Install path:** switched **A (hand-vendored `gh api` fetch) → npm devDependency**
  `@cameronsjo/artificer@0.18.1` (exact pin). The served `public/artificer/*.{css,js,json}`
...
- **Reduced-motion gap — closed at the source, not worked around:** 0.18.0's
  `artificer-whimsy.css` let `.whimsy--glacial.whimsy` `(0,2,0)!important` out-rank the
  reduced-motion `.whimsy` `(0,1,0)!important` kill switch, so the glacial wordmark kept
  drifting under `prefers-reduced-motion`. Filed upstream (#221) and **fixed in 0.18.1**;
  this consumer took the patch bump rather than shipping an app-level guard — no
  divergence to carry. (Why this is *not* its own adaptation entry: we didn't adapt.)
