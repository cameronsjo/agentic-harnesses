...
- A third **exclusion** category in `docs/methodology.md` — *orchestration layers above the loop*: **`genie`**, **`Trellis`**, **`loom`**, and **`superplane`** drive Claude Code and Codex as their execution substrate and own no model call, so the loop worth visualizing is one this repo already onboarded. Records the **dependency-manifest probe** (a project that calls a model has a model client) as the test that decided all four.
